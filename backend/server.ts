...
async function startServer() {
  ...
  app.put('/api/tecnicos/:id/toggle-estado', (req: Request, res: Response) => {
    try {
      const t = db.getTecnicoById(Number(req.params.id));
      if (!t) return res.status(404).json({ error: 'Técnico no encontrado' });
  ...
  app.post('/api/sistema/limpiar-todo', (req: Request, res: Response) => {
    try {
  ...
  app.post('/api/sistema/restablecer-ejemplos', (req: Request, res: Response) => {
    try {
